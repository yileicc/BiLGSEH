...
def main():
    ...
    with open(result_dir_path, 'a') as f:
        f.writelines('===================================================================================\n')
        f.writelines(
            '.....Hyper_parameter list:  max_epoch: %d,  hash_bit: %d, lambda1: %.3f, lambda2: %.3f,  beta: %.3f, t: %.3f, batch_size:%d \n' % (
                config.NUM_EPOCH, config.HASH_BIT, config.lambda1, config.lambda2, config.beta, config.t,  config.BATCH_SIZE))
...
def set_parameter():
    ...
    parser.add_argument('--DATASET', default='mirflickr', help='mirflickr, mscoco, nus-wide', type=str)

    parser.add_argument('--lambda1', default=10, type=float, help='loss_sim, 10')
    parser.add_argument('--lambda2', default=10, type=float, help='loss_con 1')
    parser.add_argument('--beta', default=0.01, type=float, help='0.01')
    parser.add_argument('--t', default=0.4, help='temperature=0.3', type=float)
    ...
    parser.add_argument('--LR_DIS', default=0.001, type=float, help='0.0001')
